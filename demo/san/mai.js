...
import particleSystem from './particleSystem.js'; // 导入粒子系统
...
function init() {
    ...
    scene.add(particleSystem); // 添加粒子系统到场景中
...
function initGUI(ambientLight) {
    ...
    gui.addColor(particleSystem.material, 'color').name('颜色');

    gui.add(particleSystem.position, 'x', {
        min: -10,
        max: 10,
    });

    gui.add(particleSystem.material, 'transparent').name('是否透明');
